...
Git.prototype.rm = function (files) {
  ...
  // Split the remove operation into smaller batches to avoid Windows ENAMETOOLONG
  // when transmitting a very large list of files to git.
  const batchSize = 400;
  const chunks = [];
  ...
  const executeChunk = (chunkIndex) => {
    if (chunkIndex >= chunks.length) {
      return Promise.resolve(this);
    }
    return this.exec('rm', '--ignore-unmatch', '-r', '-f', '--', ...chunks[chunkIndex]).then(() =>
      executeChunk(chunkIndex + 1),
    );
  };
